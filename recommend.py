...
class PopularityRecommendation():
    ...
    def create_p(self, t_data, u_id, i_id):
        ...
        t_data_grouped = t_data.groupby([self.i_id]).agg(
            {self.u_id: 'count'}).reset_index()
        t_data_grouped.rename(columns={'user_id': 'score'}, inplace=True)

        #Sort the songs based upon recommendation score
        t_data_sort = t_data_grouped.sort_values(
            ['score', self.i_id], ascending=[0, 1])

        #Generate a recommendation rank based upon score
        t_data_sort['Rank'] = t_data_sort['score'].rank(
...
class ItemSimilarityRecommendation():
    def __init__(self):
        self.t_data = None
        self.u_id = None
        self.i_id = None
    ...
    def construct_co_matrix(self, u_songs, a_songs):
        ...
        co_matrix = np.matrix(
            np.zeros(shape=(len(u_songs), len(a_songs))), float)
    ...
    def generate_top_r(self, user, cooccurence_matrix, a_songs, u_songs):
        ...
        s_index = sorted(((e, i) for i, e in enumerate(
        ...
        for i in range(0, len(s_index)):
            if ~np.isnan(s_index[i][0]) and a_songs[s_index[i][1]] not in u_songs and rank <= 10:
                df1.loc[len(df1)] = [user, a_songs[s_index[i][1]],
                                     s_index[i][0], rank]
                rank = rank+1
